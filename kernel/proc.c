...
int fork(void)
{
    ...
    *(np->trapframe) = *(p->trapframe);
    ...
    for (int j = 0; j < NVMA; ++j)
    {
        if (p->vm[j].used)
        {
            memmove(&np->vm[j], &p->vm[j], sizeof(p->vm[j]));
            filedup(p->vm[j].vfile);
        }
    }
...
void exit(int status)
{
    ...
    for (int i = 0; i < NVMA; ++i)
    {
        if (p->vm[i].used)
        {
            ...
            {
                filewrite(p->vm[i].vfile, p->vm[i].addr, p->vm[i].len);
            }
            fileclose(p->vm[i].vfile);
            uvmunmap(p->pagetable, p->vm[i].addr, p->vm[i].len / PGSIZE, 1);
            p->vm[i].used = 0;
        }
    }
